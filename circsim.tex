...
%En effet, l'obtention d'une chaîne parfaitement régulière par excitation facilitée nécessiterait d'exciter les atomes de Rydberg au sein d'un nuage froid très allongé, quasi-unidimensionnel.
%Ce faisant, la densité d'atomes dans l'état fondamental est fortement réduite et alors il n'y a plus suffisamment d'atomes disponibles pour être excités aux emplacements souhaités.
...
Nous présentons ici une proposition de simulateur quantique fondé sur les interactions entre des atomes de Rydberg circulaires piégés par laser.
...
%\hat{V}_{\mathrm{eff}}/h = \left(\begin{array}{cc}
...
Dans le cas plus général où les deux atomes peuvent être dans le même état, $\mathrm{nC}$ ou $\mathrm{n'C}$, le hamiltonien d'interaction prend la forme
\begin{equation}
...
%
%Étant donné que nous souhaitons simuler une chaîne de spins $1/2$, il nous faut étendre la base des états accessibles afin de considérer le cas général, où les deux atomes peuvent être dans le même état, $\mathrm{nC}$ ou $\mathrm{n'C}$.
...
Nous souhaitons maintenant écrire ce hamiltonien sur la base des opérateurs de spin.
...
Cette évolution due à l'énergie atomique et au couplage microonde s'ajoute au hamiltonien d'interaction, dont la forme est inchangée dans le référentiel tournant, et nous aboutissons à un hamiltonien total
%
\begin{equation}
...
Lorsque l'on fait varier le champ électrique $F$, parallèle à l'axe $Oz$, l'effet Stark déplace les niveaux atomiques les uns par rapport aux autres.
...
Le rapport $J_z/J$ %, ainsi que la durée de vie des niveaux circulaires,  
...
Cet effet d'inhibition a été confirmé expérimentalement pour les atomes de Rydberg circulaires \cite{MX_KELPPNER_INHIBITION,MX_KLEPPNERINHIBITION85}.
...
subit une force de van der Waals répulsive suffisante pour être éjecté de la chaîne malgré la barrière créée par le bouchon laser, comme cela est représenté en figure (\ref{fig:plugs_evap} b).
...
Ce processus de refroidissement évaporatif peut être simulé numériquement de façon quasi-classique en intégrant les équations de Newton.
...
L'ensemble initial contient $N_0=\SI{100}{}$ atomes de Rydberg circulaires.
...
Ces couplages quasi-résonants, dès lors que la distance entre les atomes est suffisamment petite, perturbent le niveau $\ket{\mathrm{nC,nC}}$ en le mélangeant à des états de paire dont l'énergie est suffisamment proche.
...
%Dans la mesure où l'on néglige le spin de l'électron, qui est très petit devant $l$ ($s=1/2 \ll l\simeq 50$), 
...
Cela s'explique par le fait que le couplage dipolaire de second ordre varie comme l'inverse des distances en énergie entre le niveau de paire considéré et les niveaux médiateurs du couplage.
...
\subsection{Autres effets limitants et estimation finale du temps de vie}
...
% : la présence de photons thermiques à température finie, les collisions avec le gaz résiduel,  la photo-ionisation due au laser de piégeage ainsi que la diffusion élastique des photons de ce même laser.
%
...
\begin{table}[!h]
	...
	\begin{tabular}{c c}
		...
		Processus dus au rayonnement du corps noir & \SI{630}{} \\
