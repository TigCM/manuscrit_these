...
\begin{figure}[!h]
\centering
\includegraphics[width=.62\linewidth]{figures/theory/Stark_60S}
...
%
\clearpage
La figure \eqref{fig:parab_mk} propose une représentation schématique des niveaux $\ket{m,k}$ de $m$ très élevé au sein d'une multiplicité $n$.
%
Le niveau de $m$ maximal dans une multiplicité donnée, $\ket{n,m=n-1,k=0} =\ket{n,l=n-1,m=l}$, est appelé le niveau circulaire $\mathrm{nC}$.
...
L'on peut tout de même introduire un opérateur potentiel effectif $\hat{V}_{\mathrm{eff}}$ pour le système à deux niveaux $\ket{ab}$ et $\ket{ba}$, qui tiendra compte du couplage éventuel au premier ordre entre $\ket{ab}$ et $\ket{ba}$ mais également des couplages de second ordre avec des états intermédiaires.
Les éléments de matrice de $\hat{V}_{\mathrm{eff}}$ sont
%
\begin{equation}\label{eq:Cab_Aab}
\begin{aligned}
&hC_{ab}=\braket{ab|\hat{V}_{\mathrm{eff}}|ab}=\braket{ba|\hat{V}_{\mathrm{eff}}|ba} \\
\text{et} &\\
&hA_{ab}=\braket{ab|\hat{V}_{\mathrm{eff}}|ba}=\braket{ab|\hat{V}_{\mathrm{eff}}|ba}.
\end{aligned}
\end{equation}
%
L'hamiltonien d'interaction s'écrit sous forme matricielle
\begin{equation}\label{eq:MatrixCab_Aab}
\hat{V}_{\mathrm{eff}}/h = \bordermatrix{~ 	&\ket{ab} 	& \ket{ba} \cr
...
\noindent Dans le cas de l'état $\ket{60\text{S},60\text{S}}$, la somme \eqref{eq:VdW_aacd} permettant de calculer le coefficient de van der Waals $C_{6,\text{60S60S}}$ est dominée par le coulage avec les paires $\ket{60\text{P}_j,59\text{P}_j}$.
...
\centering
\vspace{1em}
\includegraphics[width=.8\linewidth]{figures/theory/double_torus_FOz.png}
...
Les tores bleus représentent les orbites électroniques et les deux flèches rouges parallèles indiquent la direction du moment cinétique.
Celui-ci est aligné selon l'axe $Oz$ en raison du champ électrique $\mathrm{F}$ représenté par la flèche noire.
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.8\linewidth]{figures/theory/VdW_50C50C_1Vcm}
...
Le comportement dévie franchement de la forme de van der Waals en $1/r^6$ dès que la distance interatomique est inférieure à $\SI{10}{\micro\meter}$.
Cela est dû à la perturbation du niveau de paire, qui se mélange avec des niveaux elliptiques proches.
...
Ceux-ci ne sont pas bien décrits par le modèle de l'atome d'hydrogène en raison des effets de pénétration et de polarisabilité du c\oe ur atomique, dont le défaut quantique permet de rendre compte.
...
Nous avons introduit la base des états paraboliques, qui est d'une grande aide à leur description, en particulier en présence d'un champ électrique.
