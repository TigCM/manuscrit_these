...
\begin{figure}[!h]
\centering
\includegraphics[width=.8\linewidth]{figures/low_l/fitsim_spectresoptiques_0830126}
...
\begin{table}[!h]
	...
	\begin{tabular}{l c c}
		...
		Fréquence de Rabi max. $\Omega_0/(2\pi)$ & \SI{296}{\kHz} & \SI{60}{\kHz}\\
		Température initiale $T_0$ & $\SI{0.5}{}\pm\SI{0.15}{\uK}$ & \SI{0.8}{\uK}\\
		\midrule
		\bottomrule
 	\end{tabular}
\end{table}
...
\noindent discussion : la température ça va, c'est cool d'avoir plein de paramètres pas libres, mais quand même la fréquence de Rabi elle nous embête -> dépôt de graisse sur les fenêtres ?Mauvaise mesure de la puissance du bleu ?\\
Aussi, frequence de Rabi et température initiale ont été fittées sur le 2 us seulement !!! \\
		Aussi, on rate un peu un truc à $100 µs$ côté basses fréquences
	...
	\subsection{Caractéristiques des nuages simulés}
		...
		Aussi, sortir une simu aux temps très courts (plutôt NRyd en fonction du temps) et estimer une taille de super-atome ?

\section*{Conclusion}
		\noindent il faut prendre en compte le mouvement et le chauffage\\
		%, mais aussi (peut-être ?) les transferts thermiques vers les niveaux $p$\\
