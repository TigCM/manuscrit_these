...
Pour l'absorption de la sonde de côté par un nuage juste après la mélasse optique, nous obtenons une valeur $\alpha_{melasse} = ??????????????????????????$.
...
Troisièmement, il peut être intéressant pour des nuages très denses de ne pas calculer la densité optique $OD$ comme à l'équation \eqref{eq:Beer-Lambert_integ}, mais de s'arrêter à l'opération 
\begin{equation}
\label{eq:nolog_abs}
...
Dans le cas d'un nuage très dense, comme par exemple pour les mélasses optiques, le centre du nuage où la densité est très haute donne un signal d'absorption bruité.
L'ajustement du profil de densité est alors meilleur sur les bords du nuages.
La figure \eqref{fig:nolog_gabs} montre l'image d'une mélasse optique par absorption de la sonde de côté, traitée d'après l'équation \eqref{eq:Beer-Lambert_integ} et d'après l'équation \eqref{eq:nolog_abs}.
...
En absorption \og no-log \fg{}, le bruit dans les zones sombres est plus important, mais réduit au centre et sur les bords du nuage.
